...
import { exchangeOutlookCode } from '@/lib/calendar/token-manager';
...
export async function GET(request: NextRequest) {
  try {
    ...
    await saveCalendarSync(firestore, userId, 'outlook', {
    ...
    return NextResponse.redirect(
      new URL('/profile?calendar_connected=outlook', request.url)
    );
  } catch (error) {
